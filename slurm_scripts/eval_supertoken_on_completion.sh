...
models=( "common-pile-comma-v0.1" "meta-llama-Llama-3.2-1B" "microsoft-Phi-3-mini-4k-instruct" "gpt2" "bigscience-bloom" "facebook-xglm-564M"  "mistralai-tekken" "google-byt5-small" "google-bert-bert-base-multilingual-cased" "Qwen-Qwen3-8B"  "tokenmonster-englishcode-32000-consistent-v1"  "tiktoken-gpt-4o"  )
...
tokenizer_name="blester125/supervocab-$model"
    echo "Model: $model"
...
srun --exclusive --gres=gpu:1 --mem $MEM -o $OUT_DIR/logs/$run_name.log -e $OUT_DIR/logs/$run_name.err --nodes=1 --ntasks=1  bash -c \
' lm_eval $common_args --log_samples  \
--tasks $TOKENIZATION_ROBUSTNESS_TASKS \
--output_path $OUT_DIR ' 

# srun --exclusive --gres=gpu:1 --nodes=1 --ntasks=1 --mem $MEM 
# lm_eval \
# --tasks $EXTRA_BENCHMARKS \
# --output_path "${OUT_DIR}" --confirm_run_unsafe_code $common_args $wandb_args  &
